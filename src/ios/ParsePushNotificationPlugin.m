...
- (void)getInstallationId:(CDVInvokedUrlCommand*) command
    {
        [self.commandDelegate runInBackground:^{
            ...
            NSString *objectId = currentInstallation.installationId;
...
- (void)didReceiveRemoteNotificationWithPayload: (NSDictionary *)payload
    {
        ...
        if([aps objectForKey:@"alert"]){
            [data setObject:[aps objectForKey:@"alert"] forKey:@"alert"];
        }
        if([payload objectForKey:@"type"]){
            [data setObject:[payload objectForKey:@"type"] forKey:@"type"];
        }
        if([payload objectForKey:@"id"]){
            [data setObject:[payload objectForKey:@"id"] forKey:@"id"];
        }
        if([payload objectForKey:@"meta"]){
            [data setObject:[payload objectForKey:@"meta"] forKey:@"meta"];
        }
